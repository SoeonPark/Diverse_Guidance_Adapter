...
class ValSETrainer(BaseTrainer):
    ...
    #     other_completions_text = []
    #     if other_adapter_data is not None and other_adapter_data["completion_ids"] is not None:
    #         other_completion_ids = other_adapter_data["completion_ids"]
    #         other_completion_mask = other_adapter_data["completion_mask"]
    #         other_completions_text = self.processing_class.batch_decode(
    #             other_completion_ids, skip_special_tokens=True
    #         )
        
    #     for i, reward_func in enumerate(self.reward_funcs_diversity):
    ...
    #                         similarities = output_reward_func
    #                         mean_similarity = sum(similarities) / len(similarities)
    #                         reward_value = -mean_similarity  # Lower similarity = higher diversity reward
    #                     else:
    #                         reward_value = torch.nan
    ...
    #     diversity_scores = [-sim for sim in all_avg_similarities]  # Lower similarity = higher diversity score
    #     rewards_per_func[:, i] = torch.tensor(diversity_scores, dtype = torch.float32, device = device)
    ...
    def _calculate_rewards_diversity(
        ...
        assert other_adapter_data is not None and other_adapter_data.get("completion_ids") is not None
        ...
        # Prepare reward kwargs
        keys = [key for key in inputs[0] if key not in ["prompt", "completion", "completion_ids"]]
        reward_kwargs = {key: [example[key] for example in inputs] for key in keys}
        reward_kwargs["trainer_state"] = self.state

        # Loop over reward functions
        for i, (reward_func, reward_processing_class, reward_func_name) in enumerate(
            ...
            for curr_index, (curr_prompt, curr_completion) in enumerate(zip(prompts, completions)):
                ...
                # Non-module reward function
                    ...
                    # Format current completion
                    if isinstance(completions[curr_index], str):
                        curr_completion_formatted = [[{"role": "assistant", "content": completions[curr_index]}]]
                    else:
                        curr_completion_formatted = [completions[curr_index]]

                    # Prepare reordered inputs
                    reordered_prompts = [prompts[curr_index]] * (1 + len(other_completions_formatted))
                    reordered_completions = curr_completion_formatted + other_completions_formatted
                    ...
                    for key, value in reward_kwargs.items():
                        if key in ["other_completions", "other_completion_ids", "trainer_state"]:
                            continue
                        if isinstance(value, list) and len(value) == len(prompts):
                            reordered_kwargs[key] = [value[curr_index]] * len(reordered_prompts)

                    # Call reward function
                    output_reward_func = reward_func(
                        prompts=reordered_prompts, 
                        completions=reordered_completions, 
                        completion_ids=[completion_ids_list[curr_index]] + reward_kwargs["other_completion_ids"], 
                        **reordered_kwargs,
                    )
            ...
            # Assign to rewards tensor (여기는 루프 안!)
            rewards_per_func[:, i] = torch.tensor(all_diversity_scores, dtype=torch.float32, device=device)

        # Warning for NaN rewards
        if torch.isnan(rewards_per_func).all(dim=1).any():
            nan_row_index = torch.isnan(rewards_per_func).all(dim=1).nonzero(as_tuple=True)[0][0]
            row_reward_kwargs = {
                key: value[nan_row_index] for key, value in reward_kwargs.items() 
    ...
    def _generate_completions(
        ...
        # For Calculate and Log Policy Distance 
        if mode == "train" and self.args.calculate_policy_distance:
            if old_per_token_logps is not None:
                default_start, default_end = None, None
                for adapter_name, (start_index, end_index) in zip(self.adapter_names, adapter_boundaries):
                    if adapter_name == "default":
                        default_start, default_end = start_index, end_index
                        break
                
                if default_start is not None and default_end is not None:
                    for adapter_name, (start_index, end_index) in zip(self.adapter_names, [*adapter_boundaries]):
                        if adapter_name.startswith("specialist_"):
                            specialist_logps = old_per_token_logps[start_index:end_index]
                            specialist_mask = completion_mask[start_index:end_index]

                            actual_length = min(end_index - start_index, default_end - default_start)  # (N,)
                            default_logps = old_per_token_logps[default_start:default_end+actual_length]
                            default_mask = completion_mask[default_start:default_end+actual_length]

                            # Compute KL Divergence between specialist and default policies
                            min_len = min(specialist_logps.size(0), default_logps.size(0))
                            specialist_logps = specialist_logps[:min_len]
                            default_logps = default_logps[:min_len]
                            specialist_mask = specialist_mask[:min_len]
                            default_mask = default_mask[:min_len]
                            
                            valid_mask = (specialist_mask.bool() & default_mask.bool()).float()
                            
                            if valid_mask.sum() > 0:
                                # KL divergence: KL(specialist || default)
                                # = sum(exp(specialist_logp) * (specialist_logp - default_logp))
                                kl_per_token = torch.exp(specialist_logps) * (specialist_logps - default_logps)
                                kl_per_token = kl_per_token * valid_mask
                                
                                # Mean KL
                                kl_div = kl_per_token.sum() / valid_mask.sum()
                                
                                # Log metric
                                gathered_kl = self.accelerator.gather(kl_div)
                                self._metrics[mode][f"policy_distance/kl_div/{adapter_name}_vs_default"].append(
