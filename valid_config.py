...
@dataclass
class DGRPOConfig(TrainingArguments):
    r"""
    Configuration class for the [`DGRPOTrainer`].

    This class includes only the parameters that are specific to DGRPO training. For a full list of training arguments,
    ...
    Parameters:
        ...
        model_init_kwargs (`str`, `dict[str, Any]`, *optional*):
            ...
            argument of the [`DGRPOTrainer`] is provided as a string.
        ...
        delta (`float`, *optional*):
            Enables the upper clipping bound in two-sided DGRPO loss when set to a float. If `None` (default), standard
            DGRPO clipping is used. Recommended to be greater than `1 + ε` when enabled. This method is introduced in
        ...
        scale_rewards (`str` or `bool`, *optional*, defaults to `"group"`):
            ...
            - `False` or `"none"`: no scaling is applied. The [Dr. DGRPO
        ...
        loss_type (`str`, *optional*, defaults to `"dapo"`):
            ...
            - `"dgrpo"`: Aggregates token-level losses by normalizing over sequence length. Not recommended due to
              length bias—this approach tends to prefer shorter completions with positive advantages and longer ones
              with negative advantages.
            - `"dr_dgrpo"`: Aggregates token-level losses by normalizing with a global constant. This method was
              introduced in the [Dr. DGRPO paper](https://huggingface.co/papers/2503.20783) to eliminate length bias.
            ...
            - `"bnpo"`: Aggregates token-level losses by normalizing with the number of active token in the local
              ...
              `per_device_train_batch_size==1`, the loss is equivalent to the DGRPO loss.
    ...
    # Parameters that control the data preprocessing
    # The default value remove_unused_columns is overwritten from the parent class, because in DGRPO we usually rely on
    ...
    delta: float | None = field(
        default=None,
        metadata={
            "help": "Enables the upper clipping bound in two-sided DGRPO loss when set to a float. If `None` "
            "(default), standard DGRPO clipping is used. Recommended to be greater than `1 + ε` when enabled. This "
    ...
    scale_rewards: str = field(
        ...
        metadata={
            ...
            "`'batch'`: rewards are scaled by the standard deviation across the entire batch, as recommended in the "
            "PPO Lite paper. "
            "`False` or `'none'`: no scaling is applied. The Dr. DGRPO paper recommends not scaling rewards, as "
    ...
    loss_type: str = field(
        default="dapo",
        metadata={
            "help": "Specifies the loss formulation to use. Supported values are 'dgrpo', 'dapo', 'bnpo', and "
            "'dr_dgrpo'. "
            "'dgrpo': Aggregates token-level losses by normalizing over sequence length. Not recommended due to length "
            ...
            "'dr_dgrpo': Aggregates token-level losses by normalizing with a global constant. This method was "
            "introduced in the Dr. DGRPO paper to eliminate length bias. The value of the constant corresponds to "
    ...
    use_liger_loss: bool = field(
        default=None,
        metadata={"help": "Whether to use the Liger DGRPO loss."},
    )
    ...
    per_device_train_batch_size: int = field(
        default=1,
        metadata={
            "help": "Batch size per device during training. Overridden to 1 by default for DGRPO training."
        },
    )
    ...
    num_generations_per_diversity_adapters: int = field(
        default=3,
        metadata={
            "help": "Number of generations to sample per adapters during training."
        },
    )
    num_generations_per_base_adapter: int = field( 
        default=4,
        metadata={
            "help": "Number of generations to sample for the base adapter during training."
        },
    )
    num_guidance_adapters: int = field(
        default=2,
        metadata={
            "help": "Number of guidance adapters used during training."
        },
    )
    diversity_weight: float = field(
        default=0.5,
        metadata={
            "help": "Weight for the diversity reward in the overall reward computation."
        },
    )
    diversity_scoring_metric: str = field(
        default="bert_score",
        metadata={
            "help": "The metric used to compute diversity scores among generations. Supported metrics include "
            "Options: 'levenshtein', 'combined_rouge', 'bert-score', and 'bleu'"
        },
    )
    embedding_model_name: str = field(
        default="Qwen/Qwen3-Embedding-0.6B",
        metadata={
            "help": "The name of the embedding model used to compute embeddings for diversity scoring."
        },
    )
    masking_diversity_by_correctness: bool = field(
        default=False,
        metadata={
            "help": "Whether to consider correctness scores when computing diversity among generations."
        },
    )
    use_original_grpo_loss: bool = field(
        default=False,
        metadata={
            "help": "Whether to use the original GRPO loss instead of the modified DGRPO loss."
        },
    )
    # Weights for each Adapter: [Default Adapter, Specialist Adapter 1, Specialist Adapter 2, ...]
    ## Weights for computing diversity among generations
    correctness_weight_for_specialist: float = field(
        default=0.7,
        metadata={
            "help": "Weight for correctness when computing diversity among generations."
        },
    )
    diversity_weight_for_specialist: float = field(
        default=0.3,
        metadata={
            "help": "Weight for diversity when computing diversity among generations."
        },
    )
    ## Weights for computing diversity among generations for the default adapter
    correctness_weight_for_default: float = field(
        default=1.0,
        metadata={
            "help": "Weight for correctness when computing diversity among generations for the default adapter."
        },
    )
    diversity_weight_for_default: float = field(
        default=0.0,
        metadata={
            "help": "Weight for diversity when computing diversity among generations for the default adapter."
        },
    )
    # If to use Emphasized Entropy Bonus to Loss (ProRL)
    use_entropy_bonus: bool = field(
        default=False,
        metadata={
            "help": "Whether to use Emphasized Entropy Bonus to Loss (ProRL) during training."
        },
    )
    entropy_bonus_weight: float = field(
        default=0.01,
        metadata={
            "help": "Weight for the entropy bonus when using Emphasized Entropy Bonus to Loss (ProRL)."
        }, 
    )
    ...
    def __post_init__(self):
        ...
        if num_processes > 1:
            raise ValueError("DGRPOTrainer does not support distributed training yet.")
        
        if self.per_device_train_batch_size != 1:
            warnings.warn(
                "DGRPO is typically trained with a per_device_train_batch_size of 1. You have set it to "
                f"{self.per_device_train_batch_size}. This may lead to suboptimal results.",
                UserWarning,
                stacklevel=2,
            )
        ...
        if self.num_generations < 2:
            raise ValueError(
                "DGRPO requires at least 2 generations per prompt to calculate the advantages. You provided "
        ...
        if self.delta is not None and self.use_liger_kernel:
            raise ValueError("Liger kernel does not support two-sided DGRPO loss yet.")
