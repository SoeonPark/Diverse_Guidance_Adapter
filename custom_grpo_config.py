...
class GRPOConfig(TrainingArguments):
    ...
    # [Custom] Parameters for Adapter (De/At)taching during GRPO Training
    specialist1: Optional[bool] = field(
        default = False,
        metadata = {"help": "Whether to use the first specialist adapter during training."}
    )
    specialist2: Optional[bool] = field(
        default = False,
        metadata = {"help": "Whether to use the second specialist adapter during training."}
    )
    training_mode: Optional[str] = field(
        default = "originalGRPO",
        metadata={"help": "Training mode to use. Options are 'originalGRPOMode', 'ValSE', 'OneAdapterMode'"}
    )
    ...
    div_threshold: Optional[float] = field(
        default = 0.5,
        metadata = {"help": "Diversity Threshold for further specialization. (Less than threshold, force to use both adapters for diverse outputs.)"}
    )
